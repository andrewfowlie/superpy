...
c     read the hadronic data
      ...
      subroutine sflav_input(ilev,ierr,in_name)
c     Input read from file susy_flavor.in
      implicit double precision (a-h,o-z)
      character*18 block
      character(len=*) in_name  ! SuperPy, specify input file
...
c     open input file
      ifl = 1
      open(ifl,file=in_name,status='old')
...
c     writes SUSY_FLAVOR results to susy_flavor.out
     ...
     $     delta_mbs,dmbs_re,dmbs_im
      ...
      ifl = 6  ! SuperPy, redirect output to screen
