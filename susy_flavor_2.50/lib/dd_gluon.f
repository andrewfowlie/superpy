...
c     FILENAME: DD_GLUON.F
c     Released: 16:02:1997 (J.R.)
...
c     Hamiltonian for the d^J -> d^I + gluon decay, e.g.           c
c     b -> s gluon, at the MZ energy scale                         c
c     General form of the Hamiltonian is:                          c
c     -iH = SUM_{i=1}^5 (A^i_L H^i_L + A^i_R H^i_R) * Y^a_{al be}  c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

      subroutine dd_gluon_w(i,j,cfl,cfr)
c     W and u quark contributions (SM like)
     ...
     $     uul(3,3),uur(3,3)
      ...
      subroutine dd_gluon_h(i,j,cfl,cfr)
c     Higgs and Goldstone contributions
      ...
      subroutine dd_gluon_c(i,j,cfl,cfr)
c     chargino/up squark contributions
      ...
      subroutine dd_gluon_n(i,j,cfl,cfr)
c     neutralino-down squark contributions
      ...
      subroutine dd_gluon_g(i,j,cfl,cfr)
c     gluino-down squark contributions
      ...
      subroutine dd_gluon(i,j,cfl,cfr)
c     Full coefficients
      implicit double precision (a-h,o-z)
      double complex cfl(5),cfr(5)
      common/vpar/st,ct,st2,ct2,sct,sct2,e,e2,alpha,wm,wm2,zm,zm2,pi,sq2
      ...
      if (ih.eq.1) then
         call dd_gluon_w(i,j,cfl,cfr)
         call dd_gluon_h(i,j,cfl,cfr)
      end if
      if (ic.eq.1) call dd_gluon_c(i,j,cfl,cfr)
      if (in.eq.1) call dd_gluon_n(i,j,cfl,cfr)
      if (ig.eq.1) call dd_gluon_g(i,j,cfl,cfr)
      do k=1,5
