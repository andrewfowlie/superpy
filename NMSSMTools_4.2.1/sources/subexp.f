...
*      PROB(51) =/= 0: excluded by H/A->tautau
      ...
      DOUBLE PRECISION MH(5),XSMH(5),LCMSH(5),SIG(5)
      DOUBLE PRECISION XSMHbb(5),LCMSHbb(5),SIGbb(5)
      DOUBLE PRECISION DEL,SIGTOT,MBAR,LCMSMB
      DOUBLE PRECISION SIGTOTbb,MBARbb,LCMSMBbb
...
* Average masses weighted by the signal rates:
...
* Continue only if SIGTOT > 0 and 90<MBAR<1000 and |MH(I)-MH(I1)|<20:
             IF(SIGTOT.GT.0D0.AND.MBAR.GE.HMAS(1).AND.
     .         MBAR.LT.HMAS(NX).AND.dabs(MH(I)-MH(I1)).LE.20d0) THEN
...
* Continue only if SIGTOTbb > 0 and 90<MBARbb<1000 and |MH(I)-MH(I1)|<20:
             IF(SIGTOTbb.GT.0D0.AND.MBARbb.GE.HMAS(1).AND.
     .           MBARbb.LT.HMAS(NX).AND.dabs(MH(I)-MH(I1)).LE.20d0) THEN
