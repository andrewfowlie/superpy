...
{ double r; 
  ...
  { int l,l_;
    ...
    { double m1,m2,sqrtS;
      ...
      r*=decayPcmW(sqrtS,MV1,MV2,wV1,wV2)/decayPcmW(sqrtS,MV1,MV2,wV1,0);
    } 
  }   
  else
  {  if(kin22(PcmIn,pmass)) return 0.; 
     ...
     if(VVdecay && (AUX[nsub22].w[0] || AUX[nsub22].w[1] )) 
     {  
       double sqrtS=sqrt(PcmIn*PcmIn+pmass[0]*pmass[0]) + sqrt(PcmIn*PcmIn+pmass[1]*pmass[1]); 
       r*=decayPcmW(sqrtS,pmass[2],pmass[3],AUX[nsub22].w[0],AUX[nsub22].w[1])/decayPcm(sqrtS,pmass[2],pmass[3]);         
     }
  }
#ifdef IMPROVE
...
//passParameters(code22[k1*NC+k2]);
    ...
    { double u_min=0.,smin;
      ...
      {  int l,l_,nVV;        
         ...
         if(!AUX[nsub22].virt)
         { 
            for(l=2;l<4;l++) if(pdg[l]==23 || abs(pdg[l])==24) break; 
            if(l<4)
            {  l_=5-l; 
         ...
         {  l=AUX[nsub22].virt;
            ...
            { 
              ...
              if(cc23) smin=pmass[l_] +pmass[l]*sqrt(epsVV)/sqrt(epsVV+AUX[nsub22].w[l-2]/M_PI/pmass[l]);
            } 
         }
      }
...
/*    if(odeint(&y,1 ,Mcdm/Xt , Mcdm/X2 , 1.E-3, (Mcdm/Xt-Mcdm/X2 )/2, &XderivLn))
      { printf("problem in solving diff.equation\n"); 
        return -1;
      }*/   
    if(odeint(&y,1 ,s3_t , s3_2 , 1.E-3, (s3_2-s3_t)/2, &XderivLn))
    { /*printf("problem in solving diff.equation\n");*/
      return -1;
    }
    Yt=y;
