...
{ 
  ...
  { if(m1>m0) return 0;
    ...
    m2_p=m2;
    w2_=w2;
    return simpson( intDecay2, atan(-m2/w2), atan( ((m0-m1)*(m0-m1)  -m2*m2)/(m2*w2)),1.E-3)/M_PI/ME(m0,m1,m2); 
  }
  else if(w2==0)
  { if(m2>m0) return 0;
    m1_=m2;
    m2_p=m1;
    w2_=w1;
    return simpson( intDecay2, atan(-m1/w1), atan( ((m0-m2)*(m0-m2)-m1*m1)/(m1*w1)),1.E-3)/M_PI/ME(m0,m1,m2);
  }
  else 
  { w1_=w1;
    w2_=w2; 
    m1_p=m1;
    m2_p=m2; 
    return simpson( intDecay1, atan(-m1/w1), atan( (m0*m0-m1*m1)/(m1*w1)),1.E-3)/(M_PI*M_PI)/ME(m0,m1,m2);
  }
}
