...
C--CORRECTION FACTOR FOR SCALAR (ICASE=0) AND PSEUDOSCALAR (ICASE=1) HIGGS
      ...
      CFACQ_HDEC = 1.D0 + CKOFQ*ALPHAS_HDEC(AMH,2)/PI
      RETURN
      END
...
C--CORRECTION FACTOR FOR SCALAR HIGGS
      ...
      PI = 4*DATAN(1.D0)
      RHO = AMH**2/AMSQ**2
      CFACSQ_HDEC = 1.D0 + CKOFSQ_HDEC(RHO)*ALPHAS_HDEC(AMH,2)/PI
      RETURN
      END
...
C     KORD = 2
      ...
      G0(NF) = 1D0/2D0
      TRAN(X)=1D0+7D0/3D0*ALPHAS_HDEC(X,2)/PI
...
C     RMSQ = AMSQ/TRAN(AMSQ)
      RMSQ = AMSQ
      RUNMS_HDEC = RMSQ*CSQ(ALPHAS_HDEC(Q,2)/PI,N0)/
     .                  CSQ(ALPHAS_HDEC(Q0,2)/PI,N0)
      RETURN
      END
